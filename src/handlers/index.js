...
Program.command('handler') // sub-command name, coffeeType = type, required
  ...
  .action(async (args) => {
    ...
    if (args.deferredSettlement === true) {
      Logger.isDebugEnabled && Logger.debuf('CLI: Executing --deferredSettlement')
      const handler = {
    ...
    if (args.grossSettlement === true) {
      Logger.isDebugEnabled && Logger.debuf('CLI: Executing --grossSettlement')
      const handler = {
    ...
    if (args.rules === true) {
      Logger.isDebugEnabled && Logger.debuf('CLI: Executing --rules')
      const handler = {
        type: 'rules',
        enabled: true
      }
