...
const REPORTS_FOLDER = 'coverage_reports'
const FINAL_OUTPUT_FOLDER = 'coverage_merged'
const run = (commands) => {
...
fs.copyFileSync(
  ...
  `${REPORTS_FOLDER}/coverage-unit.json`
)
fs.copyFileSync(
  'coverage_integration/coverage-final.json',
  `${REPORTS_FOLDER}/coverage-integration.json`
)
...
run([
  ...
  `nyc report --reporter lcov --reporter json --reporter html --report-dir ${FINAL_OUTPUT_FOLDER}`
])
