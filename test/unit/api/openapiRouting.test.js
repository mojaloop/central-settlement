...
const { getBasePath, assertHandlersRegistered } = require('../../../src/api/openapiRouting')

const operation = (method, path, operationId) => ({ method, path, operationId })

Test('openapiRouting', async openapiRoutingTest => {
  await openapiRoutingTest.test('getBasePath strips a trailing slash from the servers url', async test => {
  ...
  await openapiRoutingTest.test('assertHandlersRegistered accepts a fully wired api', async test => {
    const api = {
  ...
  await openapiRoutingTest.test('assertHandlersRegistered throws naming operations with no handler', async test => {
    const api = {
      getOperations: () => [
